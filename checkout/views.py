...
def cache_checkout_data(request):
    ...
    # error message
        ...
        messages.error(
            request,
            f"Sorry, your payment can't be processed \
...
def checkout(request):
    ...
    if request.method == "POST":
            ...
            if bag["class_access_package"]:
                ...
                # error message and delete order
                    ...
                    messages.error(
                        request,
                        (
                            "The Class Access Package in your bag wasn't \
            ...
            for item_id in bag["single_classes"]:
                ...
                # error message and delete order
                    ...
                    messages.error(
                        request,
                        (
                            "One of the Exercise Classes in your bag wasn't \
                                found in our database. "
                            "Please call us for assistance!"
                        ),
                    )
        ...
        if not request.user.is_authenticated and bag["class_access_package"]:
            messages.error(
                request,
                "You can't purchase a Class Access Package\
    ...
    if not stripe_public_key:
        messages.warning(
            request,
            "Stripe Public Key is missing. Please set it \
            and try again",
        )
...
def checkout_success(request, order_number):
        ...
        if save_info:
            ...
            if user_profile_form.is_valid() and user_form.is_valid():
                try:
                    ...
                    user_form.save()
                    messages.success(
                        request, "Submitted Information Saved to Profile"
                    )
                except Exception as err:
                    messages.error(
                        request, f"User Data Not Saved. Error message: {err}"
                    )
        else:
    ...
    messages.success(
        request,
        f"Order sucessfully processed! {chr(10)}\
        Your order number is {order_number}.{chr(10)}A confirmation email \
        ...
        if request.user.is_authenticated:
            ...
            if package.type == "TK":
                if not profile.class_tokens:  # if no tokens in account
                    ...
                    profile.class_tokens += package.amount_of_tokens
            profile.package_expiry = date.today() + timedelta(package.duration)
            profile.save()
    ...
    if len(current_bag["bag_items"]["single_classes"]) > 0:
        if request.user.is_authenticated:
            ...
            list_name = order.full_name.split()
            guest_user = User(first_name=list_name[0], last_name=list_name[-1],
